...
class WorkBuddyAPI:
    ...
    def fetch_paid_packages(self, page_number: int = 1, page_size: int = 100, **extra: Any) -> dict[str, Any]:
        body = {"PageNumber": page_number, "PageSize": page_size, "Status": ["valid"], **extra}
        return self._data(self._post_json(PAID_PACKAGES, body))

    def fetch_free_packages(self, page_number: int = 1, page_size: int = 100, **extra: Any) -> dict[str, Any]:
        body = {"PageNumber": page_number, "PageSize": page_size, "Status": ["valid"], **extra}

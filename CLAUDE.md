...
| [app/auth.py](app/auth.py) | WebView 登录:加载官方授权页,轮询捕获 OpenCode/BAI/CommandCode/WorkBuddy 会话 cookie 与账号标识 |
| [app/workbuddy_api.py](app/workbuddy_api.py) | WorkBuddy user-center urllib 客户端:积分用量、套餐配额和响应解析 |
...
- **WorkBuddy 渠道**: 使用 `https://www.workbuddy.cn/profile/plans-usage` 登录并捕获全量 cookie jar；请求明细只保存 requestId、时间、模型、客户端和 credit，不保存 input/inputTrunc；积分配额走现有 30 秒账号缓存，企业版 `X-Enterprise-Id` 和积分/USD 换算不在本期范围。
...
- `workbuddy_usage`: WorkBuddy 请求级积分明细，复合主键 `(account_id, request_id)`；`credit` 为原始积分，NULL 不转换为 USD。
